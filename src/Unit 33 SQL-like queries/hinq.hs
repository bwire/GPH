...
-- trying to use common SQL-like pattern
_hinq sq jq wq = (\jd -> (\wd -> sq wd) (wq jd)) jq
...
studentEnrollmentsQ = HINQ_ (_select (\(en, st) -> (studentName st, course en))) (_join enrollments students student studentID)
...
getEnrollments courseName = runHINQ $
